...
template<class T>
Point<T> rotate(const Point<T> &a, double theta) {
    return Point(a.x * std::cos(theta) - a.y * std::sin(theta), a.x * std::sin(theta) + a.y * std::cos(theta));
}
...
template<class T>
std::pair<int, Point<T>> SegmentLineIntersection(const Line<T> &seg, const Line<T> &line) {
    int ok = cross(seg.a - line.a, line.b - line.a) * cross(seg.b - line.a, line.b - line.a) <= 0;   
    return std::make_pair(ok, ok ? lineIntersection(seg, line) : Point<T>());
}
...
template<class T>
std::pair<std::vector<Point<T>>, std::vector<Point<T>>> getHull(std::vector<Point<T>> p) {
